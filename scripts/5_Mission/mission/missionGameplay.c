...
class MissionGameplay extends MissionBase
{
	...
	static LoadMagazineIntoWeapon(Weapon_Base oWpn, Magazine oMag)
	{
	...
	static Weapon_Base CreateM4(PlayerBase oPlayer)
	{
		...
		LoadMagazineIntoWeapon(oWpn, Magazine.Cast(oPlayer.GetInventory().CreateInInventory( "Mag_STANAGCoupled_30Rnd" )));

		return oWpn;
	}

	static PlayerBase SpawnPlayer()
	{
		...
		Weapon_Base oWpn = CreateM4(oPlayer);
		oPlayer.LocalTakeEntityToHands( oWpn );
		oPlayer.SetQuickBarEntityShortcut( oWpn, 0, true );

		oWpn = CreateM4(oPlayer);
		oPlayer.SetQuickBarEntityShortcut( oWpn, 1, true );
